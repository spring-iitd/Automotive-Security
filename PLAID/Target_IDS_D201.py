...
def evaluation_metrics(all_preds, all_labels):
    ...
    plt.savefig('./CF_Results/Target_IDS_d201.png', dpi=300)
    plt.show()
...
def main():
    ...
    train_dataset_dirs = 'Surrogate_dataset_old/combined_folder_DoS_spoof'  # Add the paths to your other training directories
    train_label_files = 'Surrogate_dataset_old/combined_folder_DoS_spoof/combined_labels_dos_spoof.txt'
    
    test_dataset_dirs = 'Surrogate_dataset_old/combined_folder_DoS_spoof_test' # Same for test directories
    test_label_files = 'Surrogate_dataset_old/combined_folder_DoS_spoof_test/combined_labels_dos_spoof_test.txt'
    ...
    model_type = 'densenet201'  # Change to the desired model type
    model = models.densenet201(weights=models.DenseNet201_Weights.DEFAULT)
    model.classifier = nn.Linear(model.classifier.in_features, 2)
    model = model.to(device)
