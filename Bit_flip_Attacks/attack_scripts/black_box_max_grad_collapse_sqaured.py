...
def generate_mask(perturbed_data, modification_queue, injection_queue, top_k=12):
    """
    ...
    for _ in range(top_k):
        ...
        if not injection_queue:
            ...
            # Randomly choose 0 (modification) or 1 (injection)
        ...
        for b in range(batch_size):
            if row_type == "inj":
                # Mask ID + Data bits
                mask[b, :, row, sof_len:sof_len + id_mask_length] = 1.0
                mask[b, :, row, sof_len + id_mask_length + mid_bits_length:
                            sof_len + id_mask_length + mid_bits_length + data_bits_length] = 1.0
            else:  # modification
                # Mask Data bits only
                mask[b, :, row, sof_len + id_mask_length + mid_bits_length:
                            sof_len + id_mask_length + mid_bits_length + data_bits_length] = 1.0
...
# def bit_flip_random_attack_rgb(image, mask, data_grad, sign_data_grad):
#     """
#     Bit-flip attack for RGB CAN images.
#     - Flips pixels randomly instead of using gradient signs.
#     """
...
#             # Random-flip each ID bit with 50% probability
#             count_bit_flip = 0
#             for idx in range(ID_LEN):
#                 if random.random() < 0.5:            # 50% chance to flip this bit
#                     id_pixels[:, idx] = 1.0 - id_pixels[:, idx]
#                     count_bit_flip += 1
#             # optional: print or log
#             # print(f"Number of bitflip in ID (row {row}): {count_bit_flip}")
...
#                 if random.random() < 0.5:            # 50% chance to flip this bit
#                     data_pixels[:, idx] = 1.0 - data_pixels[:, idx]
#                     count_bit_flip += 1

#             # optional: print or log
#             # print(f"Number of bitflip in Data (row {row}): {count_bit_flip}")
...
# def bit_flip_random_data_attack_rgb(image, mask, data_grad, sign_data_grad):
#     """
#     Bit-flip attack for RGB CAN images.
#     Conditional flipping based on mask:
#         - ID bits are flipped only if mask indicates so (injection rows)
#         - Data bits are flipped if mask indicates so (both injection and modification)
#     Randomly flips each selected bit with 50% probability.

#     Args:
#         image (torch.Tensor): input image (B, C, H, W)
#         mask (torch.Tensor): binary mask (B, C, H, W)
#                              1s indicate which bits can be flipped

#     Returns:
#         perturbed_image (torch.Tensor): bit-flipped image
#     """
...
#     for b in range(B):
#         # Find all rows with any mask active in first channel
...
#             # Check if mask is active for ID or Data bits
#             id_mask_active = mask[b, 0, row, id_start:id_end].any()
#             data_mask_active = mask[b, 0, row, data_start:data_end].any()

#             # --- Flip ID bits only if mask active (injection row) ---
#             if id_mask_active:
#                 for idx in range(ID_LEN):
#                     if random.random() < 0.5:
#                         id_pixels[:, idx] = 1.0 - id_pixels[:, idx]

#             # --- Flip Data bits if mask active (both injection & modification) ---
#             if data_mask_active:
#                 for idx in range(DATA_LEN):
#                     if random.random() < 0.5:
#                         data_pixels[:, idx] = 1.0 - data_pixels[:, idx]
...
def build_modification_injection_queues(image, data_grad, rgb_pattern, max_injection_len=30, verbose=False):
    ...
    def compute_grad_for_row(row):
        ...
        masked = data_grad * mask 
        collapsed = masked.sum(dim=1)        # sum over channels -> [B, H, W]
        s = float(collapsed.sum().item())    # scalar sum over batch/cols (only this row selected)
        return float((s ** 2))
        # return float(torch.sum((data_grad * mask) ** 2).item())    #using squared sum because we are more interested in the higher abs values.
        #colllapse using normal sum and then find square sum.
    ...
    # # Sort ascending (min grad first)
    # modification_queue.sort(key=lambda x: x[0])  
    # injection_queue.sort(key=lambda x: x[0])
    ...
    # random.shuffle(modification_queue)
...
def main():
    ...
    output_path = "blackbox_final_v_0_1_sq_max_grad_collapsed_sq_k=12"

    #folder and filename to save results
    folder = './CF_Results_v_0_1_sq_max_grad_collapsed_sq_k=12'
    # folder = './CF_Results_test'
    filename = 'blackbox_final_v_0_1_sq_max_grad_collapsed_sq_k=12.png'
   ...
   # List of max_perturbations to iterate over
    ...
    packet_level_data.to_csv("./blackbox_final_v_0_1_sq_max_grad_collapsed_sq_k=12/packet_level_data.csv", index=False)
