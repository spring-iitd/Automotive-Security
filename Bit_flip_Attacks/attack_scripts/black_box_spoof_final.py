...
from plot_histogram import plot_feedback_histogram
...
def load_model(pre_trained_model_path,test_model_path):
    ...
    test_model = InceptionResNetV1(num_classes=2).to(device)
    ...
    state_dict = torch.load(pre_trained_model_path, map_location=device)
...
def load_model_pipe(surr_model_path, test_model_path):
...
def generate_mask(perturbed_data, modification_queue, injection_queue,top_k=20):
    """
    ...
    for _ in range(top_k):
        ...
        # Apply mask depending on perturbation type
        for b in range(batch_size):
            # Always mask data region
            mask[b, :, row, data_start:data_end] = 1.0
            # Mask ID region only if it's an injection
            if per_flag == "inj":
...
def bit_flip_attack_rgb(image, mask, data_grad, sign_data_grad):
    ...
    SOF_LEN, ID_LEN, MID_LEN, DATA_LEN = 1, 11, 7, 64
    id_start = SOF_LEN
    id_end = id_start + ID_LEN
    data_start = SOF_LEN + ID_LEN + MID_LEN
    ...
    for b in range(B):
        ...
        for row in rows:
            # Decide if injection (ID region masked) or modification
            per_flag = "inj" if mask[b, 0, row, id_start:id_end].any() else "mod"

            # --- ID bits flipping for injection only ---
            if per_flag == "inj":
                ...
                # Collapse gradients over channels and pick top bits
                ...
                for idx in id_top_idx:
                    grad_sign = id_signs[:, idx].sum().item()
                    if grad_sign > 0:
                        id_pixels[:, idx] = 1.0
                    elif grad_sign < 0:
                        id_pixels[:, idx] = 0.0
            ...
            # --- Data bits flipping for both injection and modification ---
            ...
            for idx in data_top_idx:
                grad_sign = data_signs[:, idx].sum().item()
                if grad_sign > 0:
                    data_pixels[:, idx] = 1.0
                elif grad_sign < 0:
                    data_pixels[:, idx] = 0.0
...
def gradient_perturbation(image, perturbed_image,mask,existing_hex_ids, packet_level_data, image_no, injection_rows, modification_rows):
    ...
    for b in range(image.shape[0]):
        ...
        for row in totalRows:
            ...
            frame_start = ('0' + (proj_bits if flag == 'injection' else '0'*ID_LEN)
                           + mid_bits + data_bits) 
            ...
            if flag == 'injection':
                ...
                packet_level_data.loc[start_index + target_index+1, "perturbation_type"] = "M"
...
def build_modification_injection_queues(image, data_grad, rgb_pattern, max_injection_len=30, verbose=False):
    ...
    #How strong are the gradients in the data bit region of this row?

    # def compute_grad_for_row_spoof(row,per_flag):
    ...
    def compute_grad_for_row_spoof(row, per_flag):
        # Create a zero mask same size as gradient
        mask = torch.zeros_like(data_grad)

        # Always apply mask to data region
        if data_start < data_end:
            mask[:, :, row, data_start:data_end] = 1

        # Additionally apply to ID region if injection type
        if per_flag == "inj" and id_start < id_end:
            mask[:, :, row, id_start:id_end] = 1

        # Compute total squared gradient energy in masked area
        grad_energy = (data_grad * mask).pow(2).sum().item()
        return float(grad_energy)

    modification_queue = [(compute_grad_for_row_spoof(r,"mod"), r) for r in modification_rows]
    injection_queue = [(compute_grad_for_row_spoof(r,"inj"), r) for r in injection_rows]
...
def Attack_procedure(model, test_model, device, test_loader, injection_type, modification_type, ep, max_injection_perturbations,output_path,bit_pattern,existing_hex_ids, start_image_number, packet_level_data):
    ...
    for data, target in test_loader:
        ...
        if final_pred == 1:
            ...
            # print("mod and inj queue len",num_mod,num_inj)
...
def main():
    ...
    # original_attack = "spoof"   # spoof or dos
    ...
    # test_dataset_dir = './test_car_hacking/dos/Test'
    # test_label_file = './test_car_hacking/dos/Test/labels.txt'
    test_dataset_dir = 'gear_dataset_images/Test'
    test_label_file = 'gear_dataset_images/Test/labels.txt'
    
    surr_model_path = "./Trained_Models/densenet161_gear.pth"
    test_model_path = "./Trained_Models/resnet_gear.pth"
    # test_model_path = "Trained_Models/dos_spoof_wisa.pth"
    
    # output_path = "blackbox_final_v_0_1_sq_k=12"
    output_path = "blackbox_gear_k20"

    #folder and filename to save results
    folder = './CF_Results/gear'
    # folder = './CF_Results_test'
    filename = 'blackbox_gear_k20.png'
    
    
    

    # packet_level_data = pd.read_csv("DoS_car_hacking_wo_stuff.csv")
    # packet_level_data = pd.read_csv("./test_car_hacking/gear/Test/track.csv")
    packet_level_data = pd.read_csv("gear_dataset_images/Test/track.csv")
    ...
    model, test_model = load_model(surr_model_path, test_model_path)
    ...
    bit_pattern = "0100001111110001000" # car hacking spoof
    # bit_pattern = "0011100000010001000"  - mirgu spoof
    ...
    #hex ids of mirgu
    # existing_hex_ids = ["0153", "0220", "0164", "018f", "0165", "0081", "0080",
    # "0316", "0260", "0329", "02a0", "0370", "0440", "043f", "02b0",
    # "01f1", "04b0", "04b1", "0545", "0382", "0350", "02c0", "05a0",
    # "05a2", "0510", "0110", "00a1", "0587", "00a0", "059b", "04f0",
    # "04f2", "0018", "0517", "0120", "0050", "05e4", "04f1", "0690",
    # "051a", "05f0", "0042", "0034", "0043", "0044"]
   ...
   # List of max_perturbations to iterate over
    ...
    packet_level_data.to_csv("./blackbox_gear_k20/packet_level_data.csv", index=False)
    # packet_level_data.to_csv("./blackbox_test/packet_level_data.csv", index=False)

    # csv_path = "blackbox_gear_k12/perturbation_summary.csv"
    # if os.path.exists(csv_path):
    #     # optionally pass custom output filenames or range
    #     plot_feedback_histogram(csv_path=csv_path,
    #                         out_plot="performance_plot_gear_k12.png",
    #                         out_stats="blackbox_gear_k12/stats.txt",
    #                         k_label="k=12",
    #                         show=False)
    # else:
