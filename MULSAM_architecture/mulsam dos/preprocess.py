...
def process_split(filename, range_tuple, split_name):
    ...
    print(f"  reading {split_name} ({filename}): Lines {start_idx} to {end_idx}...")
    ...
    # Stream file to avoid loading entire dataset into RAM
    with open(filepath, 'r') as f:
        for i, line in enumerate(f):
            # Skip rows before start_idx
            if i < start_idx: continue
            
            # Stop rows after end_idx
            if i >= end_idx: break
            ...
            parts = line.split(',')
            # Format: Timestamp, ID, DLC, Data..., Flag
            try:
                can_id_hex = parts[1]
                flag = parts[-1].strip().upper() # 'R' or 'T'
