...
def fgsm_attack_valid(image, data_grad,ep,perturbation_type, existing_hex_ids, packet_level_data, image_no):
    ...
    perturbed_image, packet_level_data = gradient_perturbation(
        image, perturbed_image, mask, existing_hex_ids, packet_level_data, image_no, 'injection')
...
def bit_flip_attack_grayscale(image, mask, data_grad, sign_data_grad):
    ...
    print("shape of perturbed image", perturbed_image)
    B, C, H, W = image.shape
    ID_LEN = 11
    MID_LEN = 7
    DATA_LEN = 64
...
def fgsm_attack_modify(image,data_grad, epsilon,perturbation_type ,ID,Data, matched_rows,selected_rows_set,bit_pattern,existing_hex_ids, packet_level_data, n_image):
    ...
    perturbed_image, packet_level_data = gradient_perturbation(
        image, perturbed_image, mask, existing_hex_ids, packet_level_data, n_image, 'modification')
...
def main():
    ...
    surr_model_path = "./Trained_Models_Grayscale_new/surrogate_ids.pth"
    test_model_path = "./Trained_Models_Grayscale_new/target_ids.pth"
    # test_model_path = "Trained_Models/dos_spoof_wisa.pth"
    
    # output_path = "blackbox_ch_final_grayscale_bitflip_with_injection_new"
    output_path = "blackbox_ch_final_test"

    #folder and filename to save results
    # folder = './CF_Results_grayscale_bitflip_with_injection_new'
   ...
   # List of max_perturbations to iterate over
    ...
    packet_level_data.to_csv("./blackbox_ch_final_grayscale_bitflip_with_injection_new/packet_level_data.csv", index=False)
    # packet_level_data.to_csv("./blackbox_ch_final_test/packet_level_data.csv", index=False)
