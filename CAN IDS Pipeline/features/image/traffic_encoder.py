...
def make_image_array(data_array, frame_type, anchor, data_rate):
    ...
    """
    #indexes = [i for i,x in enumerate(frame_type) if x == 1]
...
def generate_image(json_file_path):
    ...
    base_output_folder=  os.path.join(DIR_PATH, "..", "datasets", DATASET_NAME, MODE, TRAIN_DATASET_DIR if MODE.lower() == "train" else TEST_DATASET_DIR)
    ...
    #copy_delete_images(src_folder, dest_folder,num_images)
    ...
    # old_label_file = os.path.join(src_folder,label_file)
    # new_label_file = os.path.join(dest_folder,'DoS_test_s.txt')
    # print("OLD LABEL FILE : ", old_label_file)
    # print("NEW LABEL FILE : ", new_label_file)
    # with open(old_label_file, "r") as infile, open(new_label_file, "w") as outfile:
    ...
    # source_folder = './test_DoS_T_images'
    # label_file_path = os.path.join(source_folder, 'DoS_test_t.txt')
    # combined_folder_T = './combined_folder_DoS_images_test'
    # combined_label_file = os.path.join(combined_folder_T, 'combined_labels_dos_test.txt')
