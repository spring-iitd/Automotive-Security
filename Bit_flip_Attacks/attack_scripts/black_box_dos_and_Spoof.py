...
def load_model(pre_trained_model_path,test_model_path):
    ...
    model = torch.load(pre_trained_model_path, map_location=device, weights_only = False)
    model.to(device)
...
def generate_mask(perturbed_data, modification_queue, injection_queue, original_attack,top_k=12):
    """
    ...
    for _ in range(top_k):
        ...
        if not injection_queue:
            ...
            if inj_grad >= mod_grad:
                ...
                grad, row = modification_queue.popleft()
                modification_rows.append(row)
        # Check if modification queue is not empty
...
def bit_flip_attack_rgb(image, mask, data_grad, sign_data_grad,original_attack):
    """
...
def apply_inj_mod(data_grad, image, ep, existing_hex_ids, packet_level_data, n_image, modification_queue, injection_queue,original_attack):

    sign_data_grad = data_grad.sign()

    mask, injection_rows, modification_rows, injection_queue, modification_queue = generate_mask(image, modification_queue, injection_queue,original_attack)

    perturbed_image = bit_flip_attack_rgb(image, mask, data_grad, sign_data_grad,original_attack)
...
def perform_perturbation(test_model, data_grad, perturbed_data, ep, existing_hex_ids, packet_level_data, n_image, feedback,modification_queue, injection_queue,original_attack):
    
    perturbed_data,packet_level_data, modification_queue, injection_queue = apply_inj_mod(data_grad, perturbed_data, ep, existing_hex_ids, packet_level_data, n_image, modification_queue, injection_queue,original_attack)
...
def build_modification_injection_queues(image, data_grad, rgb_pattern, original_attack, max_injection_len=30, verbose=False):
    """
    Build two queues:
...
def Attack_procedure(model, test_model, device, test_loader, injection_type, modification_type, ep, max_injection_perturbations,output_path,bit_pattern,existing_hex_ids, start_image_number, packet_level_data,original_attack):
    all_preds = []
    all_labels = []
    ...
    for data, target in test_loader:
        ...
        if final_pred == 1:
            ...
            modification_queue, injection_queue = build_modification_injection_queues(data_denorm, data_grad, rgb_pattern, original_attack,max_injection_perturbations)
            ...
            while (modification_queue or injection_queue) and continue_perturbation:
                ...
                continue_perturbation, final_pred, data_denorm, packet_level_data, feedback,  modification_queue, injection_queue = perform_perturbation(
                    test_model, data_grad, perturbed_data, ep, existing_hex_ids, packet_level_data, n_image, feedback,modification_queue, injection_queue,original_attack)
...
def main():
    ...
    test_dataset_dir = './test_car_hacking/gear/Test'
    test_label_file = './test_car_hacking/gear/Test/labels.txt'
    
    surr_model_path = "./Trained_Models_spoof/Densenet161_spoof"
    test_model_path = "./Trained_Models_spoof/Resnet_spoof"
    # test_model_path = "Trained_Models/dos_spoof_wisa.pth"
    
    # output_path = "blackbox_final_v_0_1_sq_k=12"
    output_path = "blackbox_spoof_max_grad"

    #folder and filename to save results
    folder = './CF_Results/spoof_maxgrad'
    # folder = './CF_Results_test'
    filename = 'blackbox_spoof_maxgrad.png'
    ...
    if original_attack == 'dos':
        ...
        bit_pattern = "0100001111110001000" 
   ...
   # List of max_perturbations to iterate over
    ...
    for max_injection_perturbations in max_perturbations_list:
        ...
        preds, labels, packet_level_data = Attack_procedure(model, test_model, device, test_loader, injection_type, modification_type, epsilon, max_injection_perturbations,output_path,bit_pattern,existing_hex_ids, start_image_number, packet_level_data,original_attack)
        et = time.time()
        print("End time:", et)
        # print("Labels:", labels)
    ...
    packet_level_data.to_csv("./blackbox_spoof_max_grad/packet_level_data.csv", index=False)
