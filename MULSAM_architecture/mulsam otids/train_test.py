...
def save_confusion_matrix(cm, exp_name):
    ...
    plt.title(f'Confusion Matrix: OTIDS {exp_name}')
    plt.ylabel('Actual')
...
def train_and_evaluate(exp_name):
    print(f"\n{'='*40}")
    print(f"STARTING OTIDS TRAINING: {exp_name}")
    print(f"{'='*40}")
    ...
    try:
        ...
        print(f"Files for {exp_name} missing. Run binary_preprocess_otids.py first.")
        return
